...
START_TEST (subtract_returns_empty_string_when_maxlen_is_too_small)
{
    int maxlen = 1;
    char answer[maxlen];

    subtract("V", "I", answer, maxlen);

    ck_assert_str_eq("", answer);
}
END_TEST

START_TEST (subtract_M_minus_I_returns_CMXCIX)
{
    int maxlen = 7;
    char answer[maxlen];

    subtract("M", "I", answer, maxlen);

    ck_assert_str_eq("CMXCIX", answer);
}
END_TEST

START_TEST (subtract_XIV_minus_XVI_returns_empty_string)
{
    int maxlen = 7;
    char answer[maxlen];

    subtract("XIV", "XVI", answer, maxlen);
